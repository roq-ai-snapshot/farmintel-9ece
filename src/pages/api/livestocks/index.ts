...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getLivestocks() {
    const data = await prisma.livestock
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'livestock'));
    return res.status(200).json(data);
  }

  async function createLivestock() {
    await livestockValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.livestock.create({
      data: body,
    });
    return res.status(200).json(data);
  }
